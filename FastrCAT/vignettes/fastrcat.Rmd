...
knitr::asis_output(htmltools::includeHTML("DDY16-06_Cruise_Report.html"))
```
...
Once the make_dataframe_fc() has been run and the .csv file of the temperature, salinity, and conductivity is available you can use map_fc(). This function makes a station map. The rendered map will be written to the plot folder. It takes one argument, the path to the .csv file. 

```{r,eval=FALSE}
 map_fc("path/ to directory/ of fastrcat dataframe")
```	

Here is an example of a station map. 

```{r echo=FALSE, fig.cap= "Incorrect Output", fig.show='hold'}

knitr::include_graphics("DY1504_Station_map.png")
